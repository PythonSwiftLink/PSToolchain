...
func generateAstClass(_ cls: PyWrap.Class) -> AST.ClassDef {
	var ast_cls = cls.ast!
	...
	if let callback_index = ast_cls.body.firstIndex(where: {($0 as? AST.ClassDef)?.name == "Callbacks"}) {
...
extension AST.Arg {
	mutating func transform() {
		...
		case .Slice:
			fatalError()
		case .Subscript:
			var sub = (annotation as? AST.Subscript)!
			switch sub.name {
			case "CallableOnce":
				sub.value = AST.Name(id: "Callable")
			default: break
			}
			annotation = sub
		case .Starred:
...
extension AST.Constant {
	...
	static var property_deco: AST.Name {
		.init(id: "property")
	}
}
